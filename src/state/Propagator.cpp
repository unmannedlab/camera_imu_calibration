                                      ...
                                      Eigen::Matrix<double, 13, 1> &state_plus) /// The propagated state (q_GtoI, p_IinG, v_IinG, w_IinI)
{
    ...
    if(!have_last_prop_time_offset) {
        last_prop_time_offset = state->_calib_dt_CAMERAtoIMU->value()(0);
        have_last_prop_time_offset = true;
    }

    /// Get what our IMU-CAMERA offset should be (t_imu = t_camera + calib_dt)
    double t_off_new = state->_calib_dt_CAMERAtoIMU->value()(0);
...
//            Eigen::Vector3d a_hat = a_hat1;
            ...
            Eigen::Matrix<double,3,3> R_GtoI = camimucalib_core::quat_2_Rot(quat_GtoI);
                                     ...
                                     double timestamp) /// Time to propagate to and clone at
{
    ...
    /// Get what IMU-Camera time offset should be
    double t_off_new = state->_calib_dt_CAMERAtoIMU->value()(0);
    /// First lets construct an IMU vector of measurements we need
    double time0 = state->_timestamp + last_prop_time_offset;
    double time1 = timestamp + t_off_new;
