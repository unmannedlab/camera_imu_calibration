clear all;
clc;
imu_data = csvread('image_ts_data.csv');
...
figure(1)
p = plot(times, deltaTs, '.', 'MarkerSize', 15);
hold on;
q = plot(times, deltaTsExpected, '--', 'LineWidth', 6);
hold off;
grid;
lgd = legend([p, q], 'true \DeltaT', 'expected \DeltaT');
lgd.FontSize = 20;
ylabel('\DeltaT in ms', 'FontSize', 36);
xlabel('Time [s]', 'FontSize', 36);
title('\DeltaT in ms between Camera measurements', 'FontSize', 36);
